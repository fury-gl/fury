...
class ScrollBar(UI):
    ...
    def __init__(self, size, content_span, view_span, track_color=(1, 1, 1),
                 ...
                 track_opacity=1.0, position=(0, 0)):
        """
        Parameters
        ----------
        size : (int, int)
    ...
    def set_position(self, position):
        if self.orientation == "horizontal":
            x_position = position[0]
            x_position = max(
                x_position, self.track.position[0] + self.bar.size[0] // 2)
            x_position = min(
                x_position,
            ...
            y_position = position[1]
            y_position = max(
                y_position, self.track.position[1] + self.bar.size[1] // 2)
            y_position = min(
                y_position,
    ...
    def resize(self, size):
        ...
        if self.orientation == "vertical":
            self.bar.resize(
                (self.track.width, self._scroll_ratio * self.track.height))
            self._set_position(self.track.position)
        else:
            self.bar.resize(
                (self._scroll_ratio * self.track.width, self.track.height))
