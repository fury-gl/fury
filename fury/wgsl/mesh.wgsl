...
fn vs_main(in: VertexInput) -> Varyings {
    ...
    $$ if flat_shading
        // For flat shading, we need to calculate the normal of the face
        // and use that instead of the vertex normal. Which we can ignore binding.
        // TODO: calculate it on the CPU and bind it as a buffer.
        let i1 = i32(vii[1]);
        let i2 = i32(vii[2]);
        let edge1 = load_s_positions(i1) - raw_pos;
        let edge2 = load_s_positions(i2) - raw_pos;
        var raw_normal = normalize(cross(edge1, edge2));
    $$ else
