...
def culling(actor, back, front):
    ...
    if back and not front and not prop.GetBackfaceCulling():
        prop.SetBackfaceCulling(actor)
        
    if front and not back and not prop.GetFrontfaceCulling():
        prop.SetFrontfaceCulling(actor)
        
    if front and back:
        if not prop.GetBackfaceCulling():
            prop.SetBackfaceCulling(actor)
        if not prop.GetFrontfaceCulling():
            prop.SetFrontfaceCulling(actor)
