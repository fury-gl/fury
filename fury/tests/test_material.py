...
def test_opacity():
    ...
    test_actor = actor.square(centers, directions=(1, 1, 1), colors=(0, 0, 1))

    material.opacity(test_actor, 0.5)

    # Test opacity 'get' function
    npt.assert_equal(material.get_opacity(test_actor), 0.5)

    npt.assert_equal(material.get_opacity(test_actor),
                     material.opacity(test_actor, None))
    
    # Test opacity 'set' function
    material.set_opacity(test_actor, 0.35)
    npt.assert_equal(material.opacity(test_actor, None),
                     0.35)


def test_points():
    centers = np.array([[0, 0, 0.]])
    square_actor = actor.square(centers, directions=(1, 1, 1), colors=(0, 0, 1))

    # Test 'points' function
    npt.assert_equal(material.manifest_points(1.2), actor.GetProperty.GetPointSize())


def test_wireframe(interactive=False):
    centers = np.array([[0, 0, 0.]])
    square_actor = actor.square(centers, directions=(1, 1, 1), colors=(0, 0, 1))
    square_actor_2 = actor.square(centers, directions=(1, 0, 1), colors=(1, 0, 0))

    material.manifest_wireframe(square_actor)
    material.manifest_surface(square_actor)

    if interactive:
        scene = window.Scene()
        scene.add(square_actor)
        scene.add(square_actor_2)
...
if __name__ == '__main__':
    test_wireframe(interactive=True)
