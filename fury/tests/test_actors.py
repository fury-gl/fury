...
def test_point_cloud(interactive=True):
    # 1D points array
    points_1d = np.random.rand(3)
    npt.assert_raises(ValueError, actor.point_cloud, points_1d)

    # 3D points array
    points_3d = np.random.rand(9, 6, 3)
    npt.assert_raises(ValueError, actor.point_cloud, points_3d)

    # 2D points
    points_2d = np.random.rand(4, 2)
    npt.assert_raises(ValueError, actor.point_cloud, points_2d)

    # 4D points
    points_4d = np.random.rand(8, 4)
    npt.assert_raises(ValueError, actor.point_cloud, points_4d)

    valid_points = np.random.rand(6, 3)

    # 1D normals array
    npt.assert_raises(ValueError, actor.point_cloud, valid_points,
                      normals=points_1d)
    ...
    # Invalid normals length
    diff_len_normals = np.random.rand(3, 3)
    npt.assert_raises(ValueError, actor.point_cloud, valid_points,
                      normals=diff_len_normals)

    # Invalid normals range
    invalid_normals = np.random.rand(6, 3) * 20 - 10
    npt.assert_raises(ValueError, actor.point_cloud, valid_points,
                      normals=invalid_normals)
