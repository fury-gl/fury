...
from fury.actor import (billboard,
                        cube)
from fury.effects import (colormap_to_texture,
                          EffectManager,
                          KDE,
                          texture_to_actor,
                          window_to_texture)
...
from fury.shaders import (shader_custom_uniforms,
                          import_fury_shader)
...
points = 5.0*np.array([[0.36600749, 0.65827962, 0.53083986],
...
def test_window_to_texture(interactive=False):
    ...
    scale = np.array([[size[0]/size[1], 1.0, 0.0]])
    ...
    npt.assert_equal(
        BLENDING_MODE_DIC[blending_mode.lower()], texture.GetBlendingMode())
...
def test_texture_to_actor(interactive=False):
    ...
    texture_to_actor("test_surface.png",
                     tex_name,
                     bill,
                     blending_mode=blending_mode,
                     wrap_mode=wrap_mode,
...
def test_colormap_to_actor(interactive=False):
    ...
    scale = 3.4*np.array([[size[0]/size[1], 1.0, 0.0]])
    bill = billboard(np.array([[0.0, 0.0, 0.0]]),
                     scales=scale, fs_impl=fs_impl, fs_dec=fs_dec)
    shader_custom_uniforms(bill, "fragment").SetUniform2f("res", on_manager.size)
...
def test_kde_effect():
    kde_effect = KDE(points, bandwidths, colormap="inferno")

    npt.assert_equal(True, hasattr(kde_effect, "_offscreen_actor"))
    npt.assert_equal(True, hasattr(kde_effect, "_onscreen_actor"))
    npt.assert_equal(True, hasattr(kde_effect, "res"))

    npt.assert_array_almost_equal(np.average(points, axis=0), kde_effect.center_of_mass)
    npt.assert_equal(True, kde_effect._offscreen_actor is not None)
...
def test_kde_apply():
    ...
    kde_effect_2 = KDE(points + offset, bandwidths, colormap="viridis")

    kde_effect.apply(em)

    npt.assert_equal(True, kde_effect._onscreen_actor is not None)
    npt.assert_array_equal(size, kde_effect.res)
...
def test_call_kde():
    ...
    kde_effect = KDE(points, bandwidths, colormap="inferno")
    kde_effect.apply(em)
    kde_effect(off_manager=em.off_manager, on_manager=em.on_manager)

    off_camera = em.off_manager.scene.camera()
    off_clip_range = off_camera.GetClippingRange()
    off_cam_params = em.off_manager.scene.get_camera()
    off_size = em.off_manager.window.GetSize()

    on_camera = em.on_manager.scene.camera()
    on_clip_range = on_camera.GetClippingRange()
    on_cam_params = em.on_manager.scene.get_camera()
    on_size = em.on_manager.window.GetSize()

    npt.assert_array_equal(on_clip_range, off_clip_range)
    npt.assert_array_equal(off_cam_params[0], on_cam_params[0])
    npt.assert_array_equal(off_cam_params[1], on_cam_params[1])
    npt.assert_array_equal(off_cam_params[2], on_cam_params[2])
    npt.assert_array_equal(off_size, on_size)


def test_add(interactive=False):
    scene = window.Scene()
    scene.set_camera(position=(-24, 20, -40),
                     focal_point=(0.0,
                                  0.0,
    ...
    npt.assert_equal(True, em._n_active_effects > 0)
    npt.assert_equal(1, off_ascene.actors)
    npt.assert_equal(1, em._n_active_effects)
    npt.assert_equal(1, len(em._active_effects))
