...
normVertexMCVSOutput = vertexMC.xyz - center; // 1st Norm. [-scale, scale]
float scalingFactor = 1. / abs(normVertexMCVSOutput.x);
float size = abs(normVertexMCVSOutput.x) * 2;
normVertexMCVSOutput *= scalingFactor; // 2nd Norm. [-1, 1]
...
vec3 vertexPositionMC = center +
    cameraRightMC * billboardSize.x * normVertexMCVSOutput.x +
    cameraUpMC * billboardSize.y * normVertexMCVSOutput.y;
