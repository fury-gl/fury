...
def update_coordinates(k, w, t, d):
    a = np.linspace(-3, 3, npoints)
    b = np.sin(k*a - w*t + d)
    c = np.array([0 for i in range(npoints)])
    return a, b, c
...
def timer_callback(_obj, _event):
    global pts, pts2, colors, rate, t, dt, w, d, k
    t += dt

    x, y, z = update_coordinates(k, w, d, t)
    pts = np.array([(a, b, c) for (a, b, c) in zip(x, y, z)])
    vertices[:] = initial_vertices + \
        np.repeat(pts, no_vertices_per_point, axis=0)
    utils.update_actor(wave_actor1)

    xx, zz, yy = update_coordinates(k, w, d, t)
    pts2 = np.array([(a, b, c) for (a, b, c) in zip(xx, yy, zz)])
...
if interactive:
    showm.start()
window.record(showm.scene, size=(600, 600), out_path="em_wave.png")
