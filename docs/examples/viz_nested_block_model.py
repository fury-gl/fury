...
#####################################################################################
# Here is a simple function to extract the information of the inferred nested block
# model of  any graph-tool instance

def getNestedInference(g):
    '''Given a graph-tool instance this function extract the results obtained from  
    nested block model inference algorithm
    ...
    --------
        ...
        pos: array (float)
            A 2d array of |V|x2 size. Each row represents a  node positions in a circular layour
        labels: list (int)
            a list of  |V| int numbers. Each number represents the comunitiy which the node belongs
    '''
    state = gt.inference.minimize_nested_blockmodel_dl(g)
    
    # cts represent the control points of the Beizer curves used for graph-tool to draw the edges
    tg = gt.inference.nested_blockmodel.get_hierarchy_tree(state)[0]
    tpos = pos = gt.draw.radial_tree_layout(tg, tg.vertex(tg.num_vertices() - 1), weighted=True)
...
#####################################################################################
#  Each diferent comunity obtained from the nested block model  algorithm will have a 
# different color
from fury import colormap as cmap

comunityColors = cmap.distinguishable_colormap(nb_colors=len(np.unique(labels)))

nodeColors = np.array([comunityColors[c]
                   for c in labels])



#####################################################################################
# To plot the resuts of the inference we will need to draw curved lines in FURY. 
# To do so, we  will use the BSpline interpolation with the  control's  points  obtained 
# from the function  getNestedInference.
...
#####################################################################################
# Graph-tool stores the control point coordinates  relative to the nodes positions. 
# In order to use this control point to preform the interpolation we need first to 
# convert those control to points into real coordinates
...
#####################################################################################
# Almost finished. Now, just use the previous functions to compute the positions to 
