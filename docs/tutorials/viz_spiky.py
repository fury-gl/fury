...
The sphere actor is made of points (vertices) evenly distributing on a sphere.
...
"""
The vertices are connected with triangles in order to specify the direction of
the surface normal.
...
def timer_callback(_obj, _event):
    ...
    primitive_actor.GetProperty().SetOpacity(cnt/100.)
    showm.render()
    if cnt == 100:
        showm.exit()
