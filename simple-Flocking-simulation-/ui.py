...
class Button:
    def __init__(self, text, position=(Width - 230, 600), w=100, h=50, border=10, color=(0, 0, 0),
        ...
        if self.border > 0:
            pygame.draw.rect(screen, self.borderColor,
                             pygame.Rect(self.position[0] - self.border // 2, self.position[1] - self.border // 2,
                                         self.w + self.border, self.h + self.border))
...
class ToggleButton:
    def __init__(self, position=(Width - 200, 400), w=30, h=30, state=False, color=(40, 40, 10),
                 activeColor=(240, 140, 60)):
        self.position = position
        self.w = w
        self.h = h
        self.clicked = False
